...
FixLangevinTTM::FixLangevinTTM(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg),
...
//      for (int iznode = 0; iznode < nznodes; iznode++){
//
//        u_node[ixnode][iynode][iznode] = u_node[ixnode][iynode][iznode] / nvel[ixnode][iynode][iznode];
//        v_node[ixnode][iynode][iznode] = v_node[ixnode][iynode][iznode] / nvel[ixnode][iynode][iznode];
//        w_node[ixnode][iynode][iznode] = w_node[ixnode][iynode][iznode] / nvel[ixnode][iynode][iznode];
//
//      }
//    }
//  }
...
void FixLangevinTTM::setup(int vflag)
{
...
void FixLangevinTTM::post_force(int /*vflag*/)
{
  double **x = atom->x;
  double **v = atom->v;
  double **f = atom->f;
...
void FixLangevinTTM::post_force_setup(int /*vflag*/)
{
  double **f = atom->f;
  int *mask = atom->mask;
  int nlocal = atom->nlocal;
...
void FixLangevinTTM::post_force_respa(int vflag, int ilevel, int /*iloop*/)
{
...
void FixLangevinTTM::post_force_respa_setup(int vflag, int ilevel, int /*iloop*/)
{
...
void FixLangevinTTM::read_initial_electron_temperatures()
{
  char line[MAXLINE];
...
void FixLangevinTTM::end_of_step()
{
  if (temperature!=0){
     temperature->compute_scalar();
  }
...
//      for (int iznode = 0; iznode < nznodes; iznode++){ 
//
//        if (nvel[ixnode][iynode][iznode]==0){
//		error->all(FLERR, "Finding no atoms at one or more nodes: reduce grid resolution");
//       }
//
//	u_node[ixnode][iynode][iznode] = u_node[ixnode][iynode][iznode] / nvel[ixnode][iynode][iznode];
//        v_node[ixnode][iynode][iznode] = v_node[ixnode][iynode][iznode] / nvel[ixnode][iynode][iznode];
...
//	       }
//
//		u_node_all[ixnode][iynode][iznode] = u_node_all[ixnode][iynode][iznode] / nvel_all[ixnode][iynode][iznode];
//		v_node_all[ixnode][iynode][iznode] = v_node_all[ixnode][iynode][iznode] / nvel_all[ixnode][iynode][iznode];
//		w_node_all[ixnode][iynode][iznode] = w_node_all[ixnode][iynode][iznode] / nvel_all[ixnode][iynode][iznode];
//	     
//	      }
//	    }
//	  }
...
double FixLangevinTTM::memory_usage()
{
  double bytes = 0.0;
...
void FixLangevinTTM::grow_arrays(int ngrow)
{
...
double FixLangevinTTM::compute_vector(int n)
{
  double e_energy = 0.0;
...
void FixLangevinTTM::write_restart(FILE *fp)
{
  double *rlist;
...
void FixLangevinTTM::restart(char *buf)
{
  int n = 0;
...
int FixLangevinTTM::pack_restart(int i, double *buf)
{
  buf[0] = 4;
  buf[1] = flangevin[i][0];
  buf[2] = flangevin[i][1];
...
void FixLangevinTTM::unpack_restart(int nlocal, int nth)
{
...
int FixLangevinTTM::maxsize_restart()
{
  return 4;
}
...
int FixLangevinTTM::size_restart(int /*nlocal*/)
{
  return 4;
}
...
int FixLangevinTTM::modify_param(int narg, char **arg)
{
