...
FixLangevinTTM::FixLangevinTTM(LAMMPS *lmp, int narg, char **arg) :
  ...
  flangevin(NULL), tforce(NULL), franprev(NULL), lv(NULL), id_temp(NULL), random(NULL), ttm_flag(NULL), T_electron(NULL), T_electron_old(NULL), sum_vsq(NULL), sum_mass_vsq(NULL), sum_vsq_all(NULL), sum_mass_vsq_all(NULL),
  net_energy_transfer(NULL), net_energy_transfer_all(NULL), fp(NULL), fpr(NULL), nsum(NULL), nsum_all(NULL)
{
...
void FixLangevinTTM::setup(int vflag, int narg, char **arg)
{

  if (narg < 15 && ttm_flag==0){ 

	error->all(FLERR,"Illegal fix ttm command");}
...
/* ---------------------------------------------------------------------- */
void FixLangevinTTM::initial_integrate(int /* vflag */)
{
  double **v = atom->v;
  double **f = atom->f;
  int *mask = atom->mask;
...
void FixLangevinTTM::post_force_templated()
{
...
void FixLangevinTTM::compute_target()
{
  int *mask = atom->mask;
  int nlocal = atom->nlocal;
...
void FixLangevinTTM::omega_thermostat()
{
  double gamma1,gamma2;
...
void FixLangevinTTM::angmom_thermostat()
{
  double gamma1,gamma2;
...
void FixLangevinTTM::reset_target(double t_new)
{
...
double FixLangevinTTM::compute_scalar()
{
...
void *FixLangevinTTM::extract(const char *str, int &dim)
{
  dim = 0;
...
void FixLangevinTTM::grow_arrays(int nmax)
{
...
void FixLangevinTTM::copy_arrays(int i, int j, int /*delflag*/)
{
...
int FixLangevinTTM::pack_exchange(int i, double *buf)
{
  int n = 0;
  buf[n++] = franprev[i][0];
  buf[n++] = franprev[i][1];
...
int FixLangevinTTM::unpack_exchange(int nlocal, double *buf)
{
  int n = 0;
